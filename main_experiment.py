    ...
    for frequency in frequencies:
        ...
        for i in raw_dataset.index:
            ...
            if classification == 'binary':
                ...
                dataset_list.append(extract_features(activity, True))
                labels.append(1 if is_fall else 0)
                if is_fall:
                    dataset_list.append(extract_features(pre_fall, True))
                    labels.append(2)
                    dataset_list.append(extract_features(post_fall, True))
                    labels.append(3)
